...
vim.keymap.set({ "n", "v", "x" }, "<C-y>", '"+y', { noremap = false, silent = true, desc = "Yank to clipboard" })
vim.keymap.set({ "n", "v", "x" }, "<C-p>", '"+p', { noremap = false, silent = true, desc = "Paste from clipboard" })

-- system clipboard mappings, insert mode
-- in cmd line, has the problem that insert the clipboard content but not auto
-- render the content
vim.keymap.set({ "i", "c" }, "<C-y>", "<C-r><C-o>+", { noremap = false, silent = true, desc = "Paste from clipboard" })
