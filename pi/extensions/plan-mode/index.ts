 ...
 * - Emits mode:changed events for permission-gate integration
...
export default function planModeExtension(pi: ExtensionAPI): void {
  ...
  function togglePlanMode(ctx: ExtensionContext): void {
    ...
    if (planModeEnabled) {
      ...
      pi.events.emit("mode:changed", { mode: "plan" });
      ctx.ui.notify(`Plan mode enabled. Tools: ${PLAN_MODE_TOOLS.join(", ")}`);
    } else {
      pi.setActiveTools(NORMAL_MODE_TOOLS);
      pi.events.emit("mode:changed", { mode: "normal" });
...
- The permission gate enforces all restrictions

Ask clarifying questions using the questionnaire tool.
Use bash for web research (curl, wget).
